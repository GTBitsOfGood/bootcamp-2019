...
tictactoe.makeMove = function (board, row, column, xOrO) {
    tictactoe.errorIfNotXOrO(xOrO);
    if(board[row][column] !== " ") {
        throw "Error: " + row + " " + column + " already occupied";
    }
    let newBoard = board;
...
tictactoe.getResult = function (board) {
    ...
    for (var i = 0; i < sides.length; i++) {
        ...
        // Check for victory down (vertical)
        //
        // ex. X O O
        //     X _ _
        //     X _ _
        ...
        for (var column = 0; column < board[0].length; column++) {
            // For each column, if all boxes in the column are taken up by 'side',
            // 'side' is the winner.
