...
toolbox.replaceAll = function(string, substring, newSubstring) {
  /*var splitArray = string.split(substring);
  var newArray = [];
  for (var x = 0; x < splitArray.length - 1; x++) {
      newArray[newArray.length] = splitArray[x];
      newArray[newArray.length] = newSubstring;
      if (x == splitArray.length - 2) {
          newArray[newArray.length] = splitArray[x+1];
      }
  }
