...
toolbox.startsWith = function(string, substring) {
  ...
  return string.substring(0, n) === substring;
};
...
toolbox.replaceAll = function(string, substring, newSubstring) {
  //return string.replace(/substring/gi, newSubstring);
  let myArray = [];
  myArray = (string.split(substring).join(newSubstring))
  return myArray;
};
