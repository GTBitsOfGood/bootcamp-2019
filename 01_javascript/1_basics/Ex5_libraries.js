...
toolbox.countEven = function(n) {
  return toolbox.filter(toolbox.count(n), function isEven(a){
    return a % 2 == 0;
  });
};
...
toolbox.indexOf = function(array, item) {
  return toolbox.find(array, i => i == item)
};
...
toolbox.lastIndexOf = function(array, item) {
  let returnIndex = toolbox.find(array.reverse(), i => i == item);
  if (returnIndex == -1) {
    return returnIndex;
  }
    return (array.length - 1 - returnIndex);
};
...
toolbox.difference = function(array1, array2) {
  return toolbox.filter(array1, function notExist(n){
    return toolbox.indexOf(array2, n) == -1;
  })
};
