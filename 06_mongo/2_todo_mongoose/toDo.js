...
let ToDoitem = mongoose.model("ToDoItem", itemSchema);
...
// YOUR CODE HERE
program
.option('-t, --task <t>', 'Specifies the type of task')
...
function showTasks() {
  ...
  // YOUR CODE HERE
  ToDoitem.find({name: program.task})
  .then(results=>{
    console.log(results);
  })
  .catch(err=>{
...
function deleteTask(){
  ...
  // YOUR CODE HERE
  ToDoitem.findAndDelete({name: program.task}, function(err) {
    if(err)
